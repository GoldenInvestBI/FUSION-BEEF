...
export default function Home() {
  ...
  const filteredProducts = (() => {
    ...
    if (sortBy === 'preco_asc') {
      filtered.sort((a, b) => parseFloat(a.priceWithMarkup) - parseFloat(b.priceWithMarkup));
    } else if (sortBy === 'preco_desc') {
      filtered.sort((a, b) => parseFloat(b.priceWithMarkup) - parseFloat(a.priceWithMarkup));
    } else {
  ...
  // Products are loaded via tRPC hooks
                ...
                <Link key={product.sku} href={`/produto/${product.sku}`}>
                  <div className="group bg-card border border-border/30 rounded-lg overflow-hidden hover:shadow-2xl transition-all duration-300 hover:border-accent/50 cursor-pointer">
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-2xl font-bold text-accent mb-6">R$ {parseFloat(product.priceWithMarkup).toFixed(2)}/kg</p>
                      <Button
                        onClick={(e) => {
                          e.preventDefault();
                          handleWhatsApp(product.name, product.sku);
                        }}
